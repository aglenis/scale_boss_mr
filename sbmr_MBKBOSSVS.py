"""Code for SCALE-BOSS-MR-MB-K-BOSS-VS"""
...
class SCALE_BOSS_MR_MBKBOSSVS(BaseEstimator, UnivariateClassifierMixin):
    ...
    def __init__(self,word_size=4, n_bins=4,  window_size_list=[10], window_step_list=[1],
                 ...
                 representation= 'unigram',do_chi2=False,chi2_threshold=2,do_trend=True,k=16,coeff=0.1):
    ...
    def fit(self, X, y):
        ...
        for (curr_window_size_train,curr_window_step_train) in zip(self.window_size_list,self.window_step_list):
            ...
            final_train,ret_vocabulary,window_size_train,window_step_train,n_windows_train,tfidf,sfa,skip_gram_tfidf,relevant_return = self.process_time_series(X,y)
        ...
        if self.do_trend:
            for (curr_window_size_train,curr_window_step_train) in zip(self.window_size_list,self.window_step_list):
                ...
                final_train_trend,ret_vocabulary_trend,window_size_trend,window_step_trend,n_windows_trend,tfidf_trend,sfa_trend,skip_gram_tfidf_trend,relevant_return_trend = self.process_time_series(X_trend,y)
        ...
        for i in range(n_classes):
            curr_dataset = to_be_fitted2[y_ind == i]
            batch1 = ceil(self.coeff*curr_dataset.shape[0])
            batch1= min(batch1,1024)
            mb_kmeans = MiniBatchKMeans(n_clusters=self.k,batch_size=batch1,compute_labels=False)
            fitted = mb_kmeans.fit(curr_dataset)
            self.kmeans_list.append(fitted.cluster_centers_)
    ...
    def decision_function(self, X):
        ...
        print(self._window_size_list)
        print(self._window_step_list)
        print(self._n_windows_list)
        print(self._sfa_list)
        print(self._tfidf_list)
        print(self.skip_gram_tfidf_list)
        print(self.relevant_features_list)
        """
        for (curr_window_size,curr_window_step,curr_n_windows,curr_sfa,curr_vectorizer,curr_skipgram,curr_relevant_features) in zip(self._window_size_list,self._window_step_list,self._n_windows_list,self._sfa_list,self._tfidf_list,self.skip_gram_tfidf_list,self.relevant_features_list):
            #print("entered normal tf test")

            X_tf_normal = self.process_test_time_series(X,curr_window_size,curr_window_step,curr_n_windows,curr_sfa,curr_vectorizer,curr_skipgram,curr_relevant_features)
            #X_tf_normal = X_tf_normal[:,curr_relevant_features]
            final_test_array = np.hstack([final_test_array,X_tf_normal])

            #print(final_test_array.shape)
        if self.do_trend:
            for (curr_window_size_trend,curr_window_step_trend,curr_n_windows_trend,curr_sfa_trend,curr_vectorizer_trend,curr_skipgram_trend,curr_relevant_features_trend) in zip(self._window_size_trend_list,self._window_step_trend_list,self._n_windows_trend_list,
                       self._sfa_trend_list,self._tfidf_trend_list,self.skip_gram_tfidf_trend_list,self.relevant_features_trend_list):

                X_trend = np.diff(X)
                X_tf_trend = self.process_test_time_series(X_trend,curr_window_size_trend,curr_window_step_trend,curr_n_windows_trend,curr_sfa_trend,curr_vectorizer_trend,curr_skipgram_trend,curr_relevant_features_trend)
        ...
        #print(n_classes)
        #print(X_tf.shape)
        #to_predict = np.hstack([X_tf_normal,X_tf_trend])
        #print(final_test_array.shape)
        #ret_array = self.clf.predict(final_test_array)
        for i in range(n_classes):
            k_means_array = np.array(self.kmeans_list[i])
            similarities_array = cosine_similarity(final_test_array,k_means_array)
            #print(similarities_array.shape)
            curr_class_max = np.max(similarities_array,axis=1).reshape(-1, 1)
            #print(curr_class_max)
            classes_list.append(curr_class_max)
        ret_array = np.hstack(classes_list)
